<template>
  <div :class="classList" :ref="`m-text-area-${_uid}`">
    <label :for="_uid" v-if="label">
      {{ label }}
    </label>
    <div class="m-text-area--wrap">
      <textarea
        v-on="inputListeners"
        :value="value"
        :id="_uid"
...
export default {
  ...
  mounted() {
    if (this.autofocus) {
      const el = this.$refs[`m-text-area-${this._uid}`]
  ...
  computed: {
     ...
     */
    classList() {
      return ["m-text-area", this.dense ? "m-text-area--dense" : undefined]
    },
    ...
    classInputList() {
      return [
        "m-text-area--input",
        this.fullBox ? "m-text-area--box" : "",
        this.isInputVaid ? "" : "m-text-area--error"
      ]
    },
     ...
     */
    labelClasses() {
      return [this.isInputVaid ? "" : "m-text-area--label-error"]
    },
